( function( window, atom, require, module, undefined ) {
  ...
  commands.loadConfigFromPath = function(path) {
		...
		if(fs.existsSync(configFile)) {
			...
			var name = pathutils.dirname(path);
			var num = 0;
			while(name + (num ? `{${num}}` : '') in dirnames)
				num++;
			...
			data.commands.forEach((command) => {
				...
				var atomCommand = atom.commands.add(commandSelector, commandName, () => execute(command.command, command.arguments, command.options, command.matchs, path));
	...
	commands.unloadConfigFromName = function(name) {
		...
		delete regsiteredAtomCommands.paths[name];
		delete registeredAtomCommands.dirnames[name];
	}

  commands.activate = function( state ) {

    commands.subscriptions.add(atom.config.observe( 'dqs-shell-commands', {}, (value) => {
      // Dispose old commands
      registeredAtomCommands.config.forEach((disposable) => {
      ...
      registeredAtomCommands.config = [];

      if(value != null && value.commands != null) {
        // Register new commands
        value.commands.forEach((command) => {
          // Create an atom command for each entry
          var commandName = 'dqs-shell-commands:' + command.name;
          var commandSelector = command.selector || 'atom-workspace';
          var atomCommand = atom.commands.add(commandSelector, commandName, () => execute(command.command, command.arguments, command.options, command.matchs, 'all'));
          ...
          // Keep track of the subscriptions
          registeredAtomCommands.config.push(atomCommand);
          registeredAtomCommands.config.push(menuEntry);
          ...
          if(keymap) {
            ...
            registeredAtomCommands.config.push(entry);
            commands.subscriptions.add(entry);
          }
        });
      }
    ...
    commands.subscriptions.add(atom.project.onDidChangePaths((paths) => {
      ...
      while(keys.length > 0) {
        ...
        if(key in regsiteredAtomCommands.dirnames && newPaths.includes(registeredAtomCommands.dirnames[key]))
          newPaths.splice(oldPaths.indexOf(key), 1);
        else
          oldPaths.push(key);
      }

      // Manually dispose of commands from all paths no longer included in the project
      oldPaths.forEach(commands.unloadConfigFromPath);

      // Add commands from all paths added to the project
      newPaths.forEach(commands.loadConfigFromName);
    }));
